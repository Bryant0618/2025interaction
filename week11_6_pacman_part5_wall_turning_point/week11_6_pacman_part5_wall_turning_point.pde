...
void draw(){
 ...
 if(x+dx >= 15 && x+dx <= 600-15)x+=dx;//改變座標(不超過四面的牆)
 if(y+dy >= 15 && y+dy <= 600-15)y+=dy;//改變座標(不超過四面的牆)
 if((x-15)%30==0 && (y-15)%30==0){
   if(next==2){dx=-1;dy=0;dir=2;}//dir:2 向左PI
   if(next==0){dx=+1;dy=0;dir=0;}//dir:0 向右0度
   if(next==3){dx=0;dy=-1;dir=3;}//dir:3向上PI*1.5
   if(next==1){dx=0;dy=+1;dir=1;}//dir:1向下PI/2
   next=-1;//沒有要轉動
 }
...
int next=-1;//一開始沒有要轉動
void keyPressed(){//按方向鍵會決定next要怎麼做
