...
int x0=512,y0=512;
void loop() {
  delay(30);//要慢一點不然Processing會接不了
 int x=analogRead(A2);//把搖桿X接在A2
 int y=analogRead(A3);//把搖桿Y接在A3
 if(total<20){//想收集20筆資料
  totalX+=x;//加總要算平均值
  totalY+=y;//加總要算平均值
  count++;
  x0=totalX/count;//算平均 
 ...
 else x=(x-x0)/4.4+128;//有大的數值就減掉中間值再除四再加128
 if(abs(y-y0)<25) x=128;//數值變化太小直接放中間值128
 else y=(y-y0)/4.4+128;//有大的數值就減掉中間值再除四再加128
